...
class User(db.Model, UserMixin):
    ...
    teamname = db.Column(db.String(30), db.ForeignKey('team.name'), unique=False, nullable=False)
    password = db.Column(db.String(60), unique=False, nullable=False)
    ...
    AllChatSessions = db.relationship('ChatSessions', backref='user', lazy=True)
...
class ChatSessions(db.Model, UserMixin):
    ...
    ChatHistory = db.relationship('ChatInfo', backref='session', lazy=True)

    def __repr__(self):
        return f"ChatSessions('{self.username}', '{self.index}', '{self.uuid}')"

# Define ChatInfo model
class ChatInfo(db.Model, UserMixin):
    ...
    is_response_downloaded = db.Column(db.Boolean, nullable=False, default=False)
